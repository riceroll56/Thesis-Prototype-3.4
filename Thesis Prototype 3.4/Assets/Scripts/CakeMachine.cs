...
public class CakeMachine : MonoBehaviour
{
    public GameObject cakeMakerUI;
    ...
    void Start()
    {
        cakeMakerUI.SetActive(false);
        LoadRecipes();
    }

    private void Update()
    {
        if (!cakeMakerUI.activeSelf) return;
    ...
    private void PlaceFromInventorySlot(int slotIndex)
    {
        ...
        {
            Debug.Log("All cake machine slots are full!");
            return;
        }
        ...
        Debug.Log($"Placed {itemToPlace.GetDisplayName()} into cake machine.");
    }

    void LoadRecipes()
    {
        TextAsset jsonFile = Resources.Load<TextAsset>("cake_recipes");
        recipeDatabase = JsonUtility.FromJson<RecipeDatabase>(jsonFile.text);
        Debug.Log($"Loaded {recipeDatabase.recipes.Count} recipes from Cake JSON");
    }

    public void OnClickMakeCake()
    {
        ...
        {
            Debug.Log("Created: Exploding Cake");
            Debug.Log("Hint: What cake has yeast?");
            doughSlot.ClearSlot();
            ingredientSlot1.ClearSlot();
            ingredientSlot2.ClearSlot();
            return;
