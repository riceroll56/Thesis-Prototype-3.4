...
{
    public GameObject cookieMakerUI;
    ...
    private void Start()
    {
        cookieMakerUI.SetActive(false);
        LoadRecipes();
    }

    private void Update()
    {
        if (!cookieMakerUI.activeSelf) return;
    ...
    private void PlaceFromInventorySlot(int slotIndex)
    {
        ...
        {
            Debug.Log("All cookie machine slots are full!");
            return;
        }
        ...
        Debug.Log($"Placed {itemToPlace.GetDisplayName()} into cookie machine.");
    }

    void LoadRecipes()
    {
        TextAsset jsonFile = Resources.Load<TextAsset>("cookie_recipes");
        recipeDatabase = JsonUtility.FromJson<RecipeDatabase>(jsonFile.text);
        Debug.Log($"Loaded {recipeDatabase.recipes.Count} recipes from Cookie JSON");
    }

    public void OnClickMakeCookies()
    {
        ...
        {
            Debug.Log("Created: Tough Cookies");
        ...
        if (slotItems.Contains(Item.ItemType.Egg))
        {
            Debug.Log("Created: Tough Cookies");
            Debug.Log("Hint: The dough is too dense.");
            doughSlot.ClearSlot();
        ...
        {
            Debug.Log("Created: Exploding Cookies");
